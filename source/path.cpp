...
char SMW_Root_Data_Dir[PATH_MAX + 5] = "";
...
/****** XBOX ******/
    ...
    if (!are_paths_initialized) 
	{
		...
		ret = sysFsStat("/dev_hdd0/game/SMW00PS33/USRDIR/smw", &stat);

		if(ret == 0)
		{
			strcpy(SMW_Root_Data_Dir, "/dev_hdd0/game/SMW00PS33/USRDIR/smw");
		}
		...
		else
		{
			...
			{	
				...
				sprintf(path,"/dev_usb00%d/smw",i);
				ret = sysFsStat(path, &stat);

				if(ret == 0)
				{
					strcpy(SMW_Root_Data_Dir, "/dev_usb000/smw/");
					break;
				}
			}
		}		
